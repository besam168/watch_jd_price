...
- 2026-04-13 夜间桌面技能实战校准新增一组关键长期结论：
  1. **主屏真实鼠标有效坐标范围** 已现场校准为近似：左上 `(1,2)`，右上 `(1917,0)`，左下 `(0,1066)`，右下 `(1917,1064)`；后续这块主屏的浏览器/页面点击都应优先以这组坐标系为基线，而不是盲信 OCR 返回值。
  2. **浏览器书签栏小目标已被真实打通**，并拿到人工验收通过的真实点位：
     - `Google Gemini` ≈ `(944,100)`
     - `OpenClaw Control` ≈ `(1062,99)`
     - 上方新标签 `+` ≈ `(1000,19)`
     这三个点按顺序执行，已成功完成：`Gemini -> + -> OpenClaw Control`，实现两个网页分标签打开。
  3. `New repository` 书签这次不是靠人工报点，而是基于“顶部 OCR + 已成功书签点位反推 + 小范围热区测试”被命中，说明书签栏小目标能力已经从纯人工校点进化到**半自动推断命中**阶段。
  4. **输入区命中 + 中文输入** 也已连续实测成功：在输入区“沈万三”位置附近点击后，可成功输入 `你好`；同时浏览器地址栏点击 -> 输入 `https://www.baidu.com` -> 回车 也已成功。
  5. 当前仍需继续攻克的难点不是大目标，而是**页面表单输入框**（例如 GitHub `Repository name` 下方输入框）：已证明“标签文字坐标 ≠ 输入框热区”，简单用标签文字向下偏移还不够，后续要单独补“标签-输入框”结构化定位能力。
