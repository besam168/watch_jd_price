...
- 2026-04-03 对 `skills/tmall-genie-voice-bridge` 的本地语音闭环攻坚已取得新的长期结论：
  ...
  7. 今晚还新增了 `wake_loop.py` 持续监听原型，并试了多套唤醒词；其中 **`阿三，在吗` -> `大老板，我在`** 已在容错匹配模式下取得过一次真实触发成功，说明“本地唤醒词 + 自动应答”这条路已不是纯概念；
  8. 但要注意口径：这次成功依赖的是**容错唤醒匹配**，不是 Whisper 稳定、精确地听出了标准文本；例如成功样本里实际识别成了 `rainbow啊! 散散!在嗎??`，只是仍被规则兜中；
  9. 因此以后对外更准确的说法应是：
     - 可以说：**本地 Whisper 语音闭环 MVP 已打通，且“阿三，在吗”唤醒原型已真实成功触发过一次**；
     - 不能说：**中文语音助手已经稳定可日常使用**；
