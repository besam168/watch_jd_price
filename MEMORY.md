...
- 2026-05-07：大老板今天已明确要求把“生图 + 做 PPT”这套能力正式沉淀到长期记忆。以后默认正式口径固定为：
  ...
  8. 当前真2K尺寸必须按“已测通 / 仍观察”两类说：
     - **已真实测通**：`2048x1152`、`1536x1536`、`1152x1536`
     - **脚本已支持但当前 provider 仍观察**：`2048x1448`、`1448x2048`
  9. 做 PPT 时默认同步记住：图是真2K不等于进 PPT 就一定好看，交付时还要一起控制图框比例、contain/cover、封面裁切、留白和标题位置。
  10. 当前给别人直接复用的标准说明书固定为：`C:\Users\besam\.openclaw\workspace\手机端生图做PPT教程_正式可复制版.md`，不要再优先发旧的示例口令版。
  11. 后续对外口径必须克制：`3:2 / 2:3` 即便换成更收敛的极简 prompt 复测，当前仍返回 `HTTP 502 / upstream_error`，所以不能再说成“已稳定测通”。
...
- 2026-05-08：大老板今天已明确要求把 `desktop-web-workflow` 的 **脚本1** 固定成长期口令，后续不能再混淆成截图脚本。
  1. 以后大老板在聊天里只要说：`脚本1`，默认就是跑 `desktop-web-workflow` 的脚本1；
  2. 当前固定动作不是截图，而是：
     - 定位目标窗口 `OpenClaw Control - Google Chrome`
     - 点击输入框
     - **输入 `继续`**
     - **然后回车**
  3. 同义口令也一律按同一动作处理：
     ...
     - `起动脚本1`
     - `跑脚本1`
  4. 只有当大老板明确说：`截图`，才走 `qq-screenshot`；
  5. 因此以后默认必须记住：**“脚本1” = 打字“继续”并回车，不是截图。**
- 2026-05-08：大老板又明确要求把 **QQ 网格截图标准** 固定回工作区和长期记忆，不能继续临时手搓不同规格。
  1. 以后大老板说：`截图`，默认就是普通 QQ 截图；
  2. 以后大老板说：`网格截图`，默认就是 **QQ 标准网格截图**；
  3. 当前固定标准为：
     - capture 默认仍走 `system`
     - 网格预设固定：`quarter`
     - 样式固定：**正方小格**
     - 编号固定：**按行编号**
       ...
       - 第二行：`B1 B2 B3 ...`
       - 后面依次往下推
     - 当前实现基线：约 `40px` 一格、红色格线、白底红字小标签
  4. 这套标准已经重新补回 `skills/qq-screenshot/` 插件本体，而不再只是聊天口头约定；
  5. 以后网格截图主要用于：按钮定位、报格点击、桌面点击辅助；
  6. 默认纪律：除非大老板明确要求，否则不要随意改网格密度、标签规则或临时换成别的规格。
