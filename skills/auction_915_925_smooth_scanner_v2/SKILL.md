...
## 默认执行
```powershell
python {baseDir}/pipeline/run_v2_all_in_one.py --json
```
...
4. **run_v2_all_in_one.py**：把采样、判定、输出收成一个主入口

也就是说，这版已经不只是单快照骨架，而是已经把“连续竞价轨迹 -> 模式识别 -> 最终输出”主链路收拢起来了；后续主要是继续微调阈值和采样节奏。