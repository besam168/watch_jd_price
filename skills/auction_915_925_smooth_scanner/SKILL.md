...
description: 扫描沪深主板中小票在 09:15-09:25 集合竞价阶段，寻找价格分时轨迹平滑、接近直线且具备基本成交参与度的股票。仅使用竞价时段数据，排除创业板、科创板、北交所、ST/*ST/退市整理样式股票，并通过本地缓存增强排除上市未满60个交易日的新股/次新股，输出候选列表、指标拆解、评分、失败原因与数据源来源。适用于集合竞价纯筛选、盘前风向确认、竞价轨迹复核。
---
...
- **北交所**（如 `8xxxxx` / `4xxxxx`）
- **ST / *ST / 退市整理样式股票**
- **上市未满 60 个交易日的新股/次新股**（通过本地缓存增强过滤；缓存未命中时不阻塞全池构建）
- ETF、基金、可转债等非普通股票品种
