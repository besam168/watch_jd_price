...
- 复用 `auction_915_925_smooth_scanner` 已跑通的数据层：
  ...
  - 单批失败降级补拉
  - 统一字段输出格式
- 股票池直接复用 **`8亿股 + 150亿流通市值`** 新股票池
- 盘中版本已升级为 **多轮采样**：默认不再只看单帧快照，而是短窗口连续采样后再评分
- 最后仍以 V6 测试版的日线过滤作收口

## 正式原则
- **每个时间点都要独立扫描一次**，单独找出当时符合条件的股票。
- **不能只沿用前一时点名单做缩圈**，否则会漏掉：
  - 盘中新冒头
  - 午后回流
  - 尾盘异动
- 跨时点分析（留强 / 新冒头 / 掉队 / 板块演化）必须建立在：
  1. 各时点先独立扫描
  2. 再做结果对比
- 因此这个插件的定位不是“单次扫描器”，而是 **多时点动态扫描器**。
...
- 定时脚本 `scheduled_smallcap_scan.py` / `scheduled_smallcap_final.py` 也已切到 **pytdx + 8亿股/150亿新池**，不再走旧口。
